...
## 1.1.7 Plottingid Blockchain 2021-06-05
...
## 1.1.6 Plottingid Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /plottingid/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `plottingid configure -t true|false`).
...
## 1.1.5 Plottingid Blockchain 2021-05-09
...
## 1.1.4 Plottingid Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `plottingid keys show`. Use `plottingid keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Plottingid Blockchain 2021-05-01
...
- We changed plottingidlisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
- This is the 1.0 release of Flaxlisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [plottingidlisp.com](https://plottingidlisp.com) shortly.
...
- The `src` directory in plottingid-blockchain has been changed to `plottingid` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `plottingid/`
...
- You should review the [release notes for v1.0.2](https://github.com/Flax-Network/plottingid-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `plottingid keys`.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/plottingid-blockchain).
...
- @jespino changed `plottingid keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `plottingid keys show`.
...
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `plottingid keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/plottingid-blockchain/).
...
- Users can now pip install e.g. plottingid-blockchain==1.0.1 on most platforms.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/plottingid-blockchain).
...
- Migration with FLAX_ROOT set does not crash plottingid init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TXCH Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.plottingidnetwork.org/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `plottingid farm summary` will now display TXCH or XCH as appropriate.
...
- `plottingid farm summary` displayed some incorrect amounts.
...
- `plottingid netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `plottingid netspace`. Thanks @altendky.
- There was a typo in `plottingid plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Flax-Network/plottingid-blockchain/issues/1189)
...
- Remove `plottingid plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `plottingid keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- We now hand build Apple Silicon native binary wheels for all plottingid-blockchain dependencies and host them at [https://pypi.plottingidnetwork.org/simple](https://pypi.plottingidnetwork.org/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- Generators have been re-worked in CLVM. We added a plottingidlisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during FlaxLisp native deserialization.
...
- We have moved to a new and much higher scalability download.plottingidnetwork.org to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.plottingidnetwork.org/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.plottingidnetwork.org/latest/Setup-MacOS.dmg).
- Flax wheels not on Pypi and some dependecies not found there also are now on pypi.plottingidnetwork.org.
...
- PRs and commits to the plottingid-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TXCH farmed.
- `plottingid farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `plottingid wallet` had various bugs.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.plottingidnetwork.org/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `plottingid farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/plottingid-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `plottingid wallet` now takes get_address to get a new wallet receive address from the CLI.
- `plottingid plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of plottingid-blockchain, etc., instead of in blspy.
...
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in plottingid-blockchain.
...
- Adding Click changed the behavior of `plottingid keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.plottingidnetwork.org via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the plottingid sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
...
- Various typos and style clean ups were made to the Click CLI implementation. `plottingid -upnp f` was added to disable uPnP.
- `plottingid plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major plottingidlisp changes. TXCH from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- A new RPC api: /push_tx. Using this RPC, you can spend custom plottingidlisp programs. You need to make a SpendBundle, which includes the puzzle reveal (plottingidlisp), a solution (plottingidlisp) and a signature.
- You can now use the RPC apis to query the mempool.
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/plottingid-blockchain).
- You can obtain a new wallet receive address on the command line with `plottingid wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
- You will now see Your Harvester Network in the GUI even if you have no plots.

### Changed

- All plottingidlisp opcodes have been renumbered. This should be the last major breaking change for plottingidlisp and the clvm. There are a couple minor enhancements still needed for mainnet launch, but they may or may not require minor breaking changes. We will be restarting testnet chains on a mostly weekly basis either way.
- Node batch syncing performance was increased, and it now avoids re-validating blocks that node had already validated.
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Flax-Network/plottingid-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `plottingid -h` and `plottingid SUBCOMMAND -h` can be your guide.
...
- `plottingid plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `plottingid netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `plottingid init` only migrates release candidate directories. The versioned sub directories under `~/plottingid` will be going away before mainnet.
...
- `plottingid configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Flax-Network/plottingid-blockchain/releases/tag/1.0rc1) was well.
...
- This is the first release in our release candidate series. There are still a few things that will change at the edges but the blockchain, clvm, and plottingidlisp are in release form. We have one major change to plottingidlisp/clvm that we have chosen to schedule for the next release as in this release we're breaking the way q/quote works. We also have one more revision to the VDF that will decrease the sizes of the proofs of time. We expect a few more releases in the release candidate series.
...
- Use 'plottingid wallet get_transactions' in the command line to see your transactions.
- 'plottingid wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Flax Blockchain GUI](https://crowdin.com/project/plottingid-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/Flax-Network/plottingid-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Flax-Network/plottingid-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `plottingid plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'plottingid wallet send' now takes in TXCH or XCH as units instead of mojos.
...
- You can now send and receive transactions with the command line. Try `plottingid wallet -h` to learn more. Also, `plottingid wallet` now requires a third argument of `show`, therefor you will use `plottingid wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [plottingid blockchain gui](https://crowdin.com/project/plottingid-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `plottingid init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `plottingid init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- The GUI is now in a separate submodule repository from plottingid-blockchain, [plottingid-blockchain-gui](https://github.com/Flax-Network/plottingid-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `plottingid-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `plottingid wallet`, was incorrectly displaying TXCH balances as if they were Coloured Coins.
...
- `plottingid plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Flax-Network/plottingid-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Flax-Network/plottingid-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Flax-Network/plottingid-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `plottingid plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- We have removed the '-r' flag from `plottingid` as it was being overridden in most cases by the `-r` for restart flag to `plottingid start`. Use `plottingid --root-path` instead.
- `plottingid -h` now recommends `plottingid netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `plottingid show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `plottingid configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `plottingid plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `plottingid plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `plottingid show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `plottingid show` handles sub block 0 better.
...
- `plottingid plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `plottingid plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `plottingid show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `plottingid configure --enable-upnp`.
...
- `plottingid configure` now lets you enable or disable uPnP.
...
- `plottingid netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `plottingid show` of the GUI use `plottingid netspace -d 1000` as `plottingid netspace` defaults to `-d 192` which is one hour.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [plottingidnetwork.org](https://plottingidnetwork.org/). Among the improvements this gives the Flax blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- You can now specify which private key to use for `plottingid plots create`. After obtaining the fingerprint from `plottingid keys show`, try `plottingid plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- There is a new cli command - `plottingid configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `plottingid configure -h`. Note that if plottingid services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.plottingid/VERSION/config/`
...
- `plottingid show -s` has been refactored to support the new consensus.
- `plottingid netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.plottingid/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run plottingid-blockchain.
...
- `plottingid show -s` now is less brain dead when a node is initially starting to sync.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Flax-Network/plottingid-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install plottingid-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/Flax-Network/plottingid-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Flax-Network/plottingid-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Flax-Network/plottingid-blockchain/tree/dev/electron-react/src/locales) of the plottingid-blockchain repository.
- `plottingid check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- The rpc interfaces of all plottingid services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test plottingid since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve plottingid-blockplottingidn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Flax Network at backup.plottingidnetwork.org. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Flax backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.plottingidnetwork.org, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test plottingid are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.plottingid` directory.
...
- `plottingid init` now automatically discovers previous releases in each new release.

### Fixed

- `plottingid show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `plottingid show -w` with thanks to @pyl on Discord.
...
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (plottingid run farmer-only, wallet-only, timelord-only)
...
- `plottingid show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `plottingid show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
...
- See wallet balances in command line: `plottingid show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `plottingid keys sign` and `plottingid keys verify` to allow farmers to certify their ownership of keys.
...
- `plottingid start wallet-server` changed to `plottingid start wallet`, for consistency.
...
- Removed legacy scripts such as plottingid-stop-server, plottingid-restart-harvester, etc.
...
- The plottingid executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./plottingid -h` from
...
"plottingid plots create" and "plottingid plots check".
- Add plot directories to config.yaml from the cli with "plottingid plots add".
...
- Test plots for the simulation and testing harness now go into `~/.plottingid/test-plots/`
...
- We added additional improvements and more RPCs to the start daemon and various services to continue to make plottingid start/stop reliable cross platform.
...
- Starting plottingid now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'plottingid keys' command line.
...
- We added a `plottingid keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the plottingid-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `plottingid netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `plottingid-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `plottingid-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `plottingid-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `plottingid-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `plottingid init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `plottingid start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `plottingid-start-harvester`.
...
- The user interface now refers to plottingid coins with their correct currency code of XCH.
...
- Now you can run estimates of the total space currently farming the network. Try `plottingid netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during plottingid init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `plottingid show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `plottingid show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Flax-Network/plottingid-blockchain/wiki) and placed in [INSTALL](https://github.com/Flax-Network/plottingid-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Flax-Network/plottingid-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of plottingid action commands. You can run `plottingid version` or `plottingid start node` for example. Just running `plottingid` will show you more options. However `plottingid-create-plots` continues to use the hyphenated form. Also it's now `plottingid generate keys` as another example.
- Flax start commands like `plottingid start farmer` and `plottingid stop node` now keep track of process IDs in a run/ directory in your configuration directory. `plottingid stop` is unlikely to work on Windows native for now. If `plottingid start -r node` doesn't work you can force the run/ directory to be reset with `plottingid start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Flax-Network/plottingid-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `plottingid start wallet-gui` has been chaned to `plottingid start wallet` which launches but the GUI and server on MacOS and Linux. `plottingid start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Flax-Network/plottingid-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `plottingid stop all` command sometimes fails, use `plottingid-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `plottingid init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `plottingid-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Flax-Network/plottingid-blockchain/wiki/Updating-beta-software). If you've set `$FLAX_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `plottingid show`.

### Changed

- `plottingid-restart-harvester` has been renamed from `plottingid-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.plottingid/beta-1.0b3/config/plots.yaml` after migrating
...
- We have revamped the plottingid management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `plottingid-start-farmer &`. The [README.md](https://github.com/Flax-Network/plottingid-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.plottingid/release/config, plots/ db/, wallet/ etc. This can be overridden by `export FLAX_ROOT=~/.plottingid` for example which would then put the plots directory in `HomeDirectory/.plottingid/plots`.
- The libraries plottingid-pos, plottingid-fast-vdf, and plottingid-bip-158 have been moved to their own repositories: [chiapos](https://github.com/Flax-Network/chiapos), [chiavdf](https://github.com/Flax-Network/chiavdf), and [chaibip158](https://github.com/Flax-Network/chiabip158). They are brought in by plottingid-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Flax-Network/bls-signatures).
...
- `plottingid-regenerate-keys` has been renamed `plottingid-generate-keys`.
...
- Flax fullnode, farmer and harvester now default to logging to plottingid.log in the plottingid-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- Added color logging
- Now plottingid_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to plottingid_full node, plottingid_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new plottingid services were started.
...
- More details on the release at [https://www.plottingidnetwork.org/developer/](https://www.plottingidnetwork.org/developer/)

[unreleased]: https://github.com/Flax-Network/plottingid-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Flax-Network/plottingid-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Flax-Network/plottingid-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Flax-Network/plottingid-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Flax-Network/plottingid-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Flax-Network/plottingid-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Flax-Network/plottingid-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Flax-Network/plottingid-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Flax-Network/plottingid-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Flax-Network/plottingid-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Flax-Network/plottingid-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Flax-Network/plottingid-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Flax-Network/plottingid-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Flax-Network/plottingid-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Flax-Network/plottingid-blockchain/releases/tag/Alpha-1.0
