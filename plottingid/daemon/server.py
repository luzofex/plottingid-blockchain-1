...
from plottingid.cmds.init_funcs import plottingid_init
from plottingid.daemon.windows_signal import kill
from plottingid.server.server import ssl_context_for_root, ssl_context_for_server
from plottingid.ssl.create_ssl import get_mozzila_ca_crt
from plottingid.util.plottingid_logging import initialize_logging
from plottingid.util.config import load_config
from plottingid.util.json_util import dict_to_json_str
from plottingid.util.path import mkdir
from plottingid.util.service_groups import validate_service
from plottingid.util.setproctitle import setproctitle
from plottingid.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Plottingid.")
    quit()
...
service_plotter = "plottingid plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "plottingid": "plottingid",
        "plottingid_wallet": "start_wallet",
        "plottingid_full_node": "start_full_node",
        "plottingid_harvester": "start_harvester",
        "plottingid_farmer": "start_farmer",
        "plottingid_introducer": "start_introducer",
        "plottingid_timelord": "start_timelord",
        "plottingid_timelord_launcher": "timelord_launcher",
        "plottingid_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("plottingid daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered PLOTTINGID_ROOT
    os.environ["PLOTTINGID_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with PLOTTINGID_ROOT: {os.environ['PLOTTINGID_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    plottingid_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("plottingid_daemon")
...
def main() -> int:
    from plottingid.util.default_root import DEFAULT_ROOT_PATH
