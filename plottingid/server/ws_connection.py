...
from plottingid.cmds.init_funcs import plottingid_full_version_str
from plottingid.protocols.protocol_message_types import ProtocolMessageTypes
from plottingid.protocols.shared_protocol import Capability, Handshake
from plottingid.server.outbound_message import Message, NodeType, make_msg
from plottingid.server.rate_limits import RateLimiter
...
from plottingid.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from plottingid.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSPlottingidConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            outbound_handshake = make_msg(
                ...
                Handshake(
                    'plottingid-' + network_id,
                    protocol_version,
                    plottingid_full_version_str(),
                    uint16(server_port),
            ...
            if ProtocolMessageTypes(inbound_handshake_msg.type) != ProtocolMessageTypes.handshake:
                raise ProtocolError(Err.INVALID_HANDSHAKE)
            if inbound_handshake.network_id != 'plottingid-' + network_id:
