...
from plottingid.wallet.puzzles.puzzle_utils import (
...
from plottingid.wallet.secret_key_store import SecretKeyStore
from plottingid.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_plottingid(self, plottingid_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if plottingid_amount < 0:
            utxos = await self.select_coins(abs(plottingid_amount), exclude)
        else:
        ...
        plottingid_amount = spend_value + plottingid_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": plottingid_amount}]
