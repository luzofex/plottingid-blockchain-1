...
from plottingid.protocols import wallet_protocol
from plottingid.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from plottingid.protocols.protocol_message_types import ProtocolMessageTypes
from plottingid.protocols.wallet_protocol import (
...
from plottingid.server.node_discovery import WalletPeers
...
from plottingid.server.ws_connection import WSPlottingidConnection
...
from plottingid.util.errors import Err, ValidationError
from plottingid.util.ints import uint32, uint128
from plottingid.util.keychain import Keychain
from plottingid.util.lru_cache import LRUCache
from plottingid.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from plottingid.wallet.settings.settings_objects import BackupInitialized
...
from plottingid.wallet.wallet_blockchain import ReceiveBlockResult
from plottingid.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[PlottingidServer]
    log: logging.Logger
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'plottingid keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSPlottingidConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSPlottingidConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSPlottingidConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSPlottingidConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSPlottingidConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSPlottingidConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
