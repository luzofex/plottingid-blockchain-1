...
from plottingid.rpc.farmer_rpc_client import FarmerRpcClient
from plottingid.rpc.full_node_rpc_client import FullNodeRpcClient
from plottingid.rpc.harvester_rpc_client import HarvesterRpcClient
...
from plottingid.util.misc import format_minutes
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total plottingid farmed: {amounts['farmed_amount'] / units['plottingid']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['plottingid']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['plottingid']}")
        print(f"Last height farmed: {amounts['last_height_farmed']}")
    else:
        print("Total plottingid farmed: Unknown")
    ...
    print("Note: log into your key using 'plottingid wallet show' to see rewards for each key")
