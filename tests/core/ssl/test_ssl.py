...
from plottingid.protocols.shared_protocol import protocol_version
from plottingid.server.outbound_message import NodeType
from plottingid.server.server import FlaxServer, ssl_context_for_client
from plottingid.server.ws_connection import WSFlaxConnection
from plottingid.ssl.create_ssl import generate_ca_signed_cert
from plottingid.types.peer_info import PeerInfo
from plottingid.util.block_tools import test_constants
from plottingid.util.ints import uint16
...
class TestSSL:
    ...
    async def test_farmer(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            farmer_server.plottingid_ca_crt_path.read_bytes(), farmer_server.plottingid_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            farmer_server.plottingid_ca_crt_path, farmer_server.plottingid_ca_crt_path, pub_crt, pub_key
        )
        connected = await establish_connection(farmer_server, 12312, ssl_context)
    ...
    async def test_full_node(self, wallet_node):
        ...
        generate_ca_signed_cert(
            full_node_server.plottingid_ca_crt_path.read_bytes(),
            full_node_server.plottingid_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            full_node_server.plottingid_ca_crt_path, full_node_server.plottingid_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_wallet(self, wallet_node):
        ...
        generate_ca_signed_cert(
            wallet_server.plottingid_ca_crt_path.read_bytes(), wallet_server.plottingid_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            wallet_server.plottingid_ca_crt_path, wallet_server.plottingid_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_harvester(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            harvester_server.plottingid_ca_crt_path.read_bytes(),
            harvester_server.plottingid_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            harvester_server.plottingid_ca_crt_path, harvester_server.plottingid_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_introducer(self, introducer):
        ...
        # Create not authenticated cert
        pub_crt = introducer_server.plottingid_ca_key_path.parent / "p2p.crt"
        pub_key = introducer_server.plottingid_ca_key_path.parent / "p2p.key"
        generate_ca_signed_cert(
            introducer_server.plottingid_ca_crt_path.read_bytes(),
            introducer_server.plottingid_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            introducer_server.plottingid_ca_crt_path, introducer_server.plottingid_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_timelord(self, timelord):
        ...
        generate_ca_signed_cert(
            timelord_server.plottingid_ca_crt_path.read_bytes(),
            timelord_server.plottingid_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            timelord_server.plottingid_ca_crt_path, timelord_server.plottingid_ca_crt_path, pub_crt, pub_key
        )
