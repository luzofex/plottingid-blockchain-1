...
from plottingid.server.server import PlottingidServer, ssl_context_for_client
...
async def disconnect_all_and_reconnect(server: PlottingidServer, reconnect_to: PlottingidServer) -> bool:
...
async def add_dummy_connection(server: PlottingidServer, dummy_port: int) -> Tuple[asyncio.Queue, bytes32]:
    ...
    generate_ca_signed_cert(
        server.plottingid_ca_crt_path.read_bytes(), server.plottingid_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.plottingid_ca_crt_path, server.plottingid_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: PlottingidServer, server_2: PlottingidServer) -> WSPlottingidConnection:
    """
